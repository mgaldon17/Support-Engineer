...
class Handler(BaseHTTPRequestHandler):
    ...
    def _serve_file(self, rel: str) -> None:
        ...
        if clean == "index.html":
            base = _HERE
            target = (_HERE / "index.html").resolve()
        elif clean.startswith("assets/"):
            # Reject ambiguous/suspicious path syntax before filesystem access.
            if "\\" in clean:
                return self._json({"error": "forbidden"}, 403)
            asset_rel = clean[len("assets/"):]
            parts = [p for p in asset_rel.split("/") if p]
            if not parts or any(p in (".", "..") for p in parts):
                return self._json({"error": "forbidden"}, 403)
            base = (_HERE / "assets").resolve()
            target = (base / Path(*parts)).resolve()
        else:
            return self._json({"error": "forbidden"}, 403)

        if not target.is_relative_to(base):   # symlink-safe containment check
            return self._json({"error": "forbidden"}, 403)
