...
async def _recall(prompt: str) -> list:
    ...
    return await store.search(prompt, limit=cfg.lesson_search_limit)


def _format(lessons: list) -> str:
    lines = [
        "Relevant lessons from memory (procedures learned/curated previously). "
        "Judge by MEANING, not wording; if one covers this task, follow it:",
    ]
    for l in lessons:
        lines.append(f"- [{l.lesson_id}] {l.title} (reuse={l.reuse}, failures={l.failure_count})")
